<div class="container my-5">
    <div class="row justify-content-center">
        <div class="col-lg-10">
            <nav aria-label="breadcrumb" class="mb-3">
                <ol class="breadcrumb small mb-0">
                    <li class="breadcrumb-item"><a href="<?= BASE_URL ?>" class="text-decoration-none text-muted">Beranda</a></li>
                    <li class="breadcrumb-item active text-primary fw-semibold" aria-current="page">Jadwal & Pedoman</li>
                </ol>
            </nav>

            <div class="mb-4">
                <h1 class="h3 fw-bold text-dark mb-1">Jadwal & Pedoman BAAK</h1>
                <p class="text-muted">Dokumen resmi operasional akademik. Silakan hubungi BAAK untuk mendapatkan dokumen lengkap.</p>
            </div>

            <div class="row g-3 mb-4">
                <div class="col-md-6">
                    <div class="card border-0 shadow-sm h-100 rounded-3">
                        <div class="card-body d-flex align-items-center p-3">
                            <div class="bg-danger bg-opacity-10 text-danger rounded-circle d-flex align-items-center justify-content-center me-3" style="width: 48px; height: 48px; flex-shrink: 0;">
                                <i class="bi bi-file-earmark-pdf-fill fs-5"></i>
                            </div>
                            <div>
                                <div class="fw-semibold text-dark">Daftar Mata Kuliah per Prodi</div>
                                <small class="text-muted">Update Semester Ganjil 2026</small>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="card border-0 shadow-sm h-100 rounded-3">
                        <div class="card-body d-flex align-items-center p-3">
                            <div class="bg-danger bg-opacity-10 text-danger rounded-circle d-flex align-items-center justify-content-center me-3" style="width: 48px; height: 48px; flex-shrink: 0;">
                                <i class="bi bi-file-earmark-pdf-fill fs-5"></i>
                            </div>
                            <div>
                                <div class="fw-semibold text-dark">Koordinator Mata Kuliah</div>
                                <small class="text-muted">Data dosen penanggung jawab</small>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="card border-0 shadow-sm h-100 rounded-3">
                        <div class="card-body d-flex align-items-center p-3">
                            <div class="bg-danger bg-opacity-10 text-danger rounded-circle d-flex align-items-center justify-content-center me-3" style="width: 48px; height: 48px; flex-shrink: 0;">
                                <i class="bi bi-file-earmark-pdf-fill fs-5"></i>
                            </div>
                            <div>
                                <div class="fw-semibold text-dark">Kalender Akademik</div>
                                <small class="text-muted">Jadwal resmi perkuliahan</small>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="card border-0 shadow-sm h-100 rounded-3">
                        <div class="card-body d-flex align-items-center p-3">
                            <div class="bg-danger bg-opacity-10 text-danger rounded-circle d-flex align-items-center justify-content-center me-3" style="width: 48px; height: 48px; flex-shrink: 0;">
                                <i class="bi bi-file-earmark-pdf-fill fs-5"></i>
                            </div>
                            <div>
                                <div class="fw-semibold text-dark">Jadwal Seminar & Ujian</div>
                                <small class="text-muted">Magang, Sempro, TA, Ujian TA</small>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="card border-0 bg-primary bg-opacity-10 rounded-3 mb-4">
                <div class="card-body p-4">
                    <h5 class="fw-bold text-dark"><i class="bi bi-info-circle-fill me-2 text-primary"></i>Cara Mendapatkan Dokumen</h5>
                    <p class="text-secondary mb-0">
                        Untuk mengunduh dokumen di atas, silakan kunjungi kantor BAAK atau hubungi kami melalui kontak yang tersedia di bawah.
                    </p>
                </div>
            </div>

            <div class="row g-3">
                <div class="col-md-6">
                    <div class="card border-0 bg-white shadow-sm h-100 rounded-3">
                        <div class="card-body d-flex align-items-center p-3">
                            <div class="bg-primary bg-opacity-10 text-primary rounded-circle d-flex align-items-center justify-content-center me-3" style="width: 40px; height: 40px; flex-shrink: 0;">
                                <i class="bi bi-envelope-fill"></i>
                            </div>
                            <div>
                                <small class="fw-semibold text-dark">Email</small><br>
                                <small class="text-muted">baak@politekniknest.ac.id</small>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="card border-0 bg-white shadow-sm h-100 rounded-3">
                        <div class="card-body d-flex align-items-center p-3">
                            <div class="bg-primary bg-opacity-10 text-primary rounded-circle d-flex align-items-center justify-content-center me-3" style="width: 40px; height: 40px; flex-shrink: 0;">
                                <i class="bi bi-geo-alt-fill"></i>
                            </div>
                            <div>
                                <small class="fw-semibold text-dark">Lokasi</small><br>
                                <small class="text-muted">Gedung Rektorat Lantai 1</small>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
